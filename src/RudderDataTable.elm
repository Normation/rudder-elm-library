    ...
    case msg of
        ...
        UpdateFilterMsg filter ->
            ...
            ( Model
                { model
                    | filter = filter
                    , data = List.filter (model.applyFilter (createPredicate filter)) model.data
                }
            , effects
            , Nothing
            )
...
refreshButton : RefreshButtonOptions msg -> Html (Msg msg)
refreshButton options =
    case options of
        RefreshButtonOptions attrs ->
            button (onClick RefreshMsg :: attrs) [ text "refresh" ]
